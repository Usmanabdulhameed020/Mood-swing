...
export default function NowPlayingModal({ song, isOpen, onClose }) {
  ...
  const startPolling = useCallback(() => {
    ...
    intervalRef.current = setInterval(() => {
      ...
      try {
        ...
        if (dur > 0) setOnlineDuration(dur);
  ...
  }, [syncedLyrics, setOnlineCurrentTime, setOnlineDuration]);
  ...
  useEffect(() => {
    if (syncedLyrics && isOnlinePlaying && playerRef.current) {
      startPolling();
    }
  }, [syncedLyrics, isOnlinePlaying, startPolling]);

  const onPlayerReady = (event) => {
    playerRef.current = event.target;
    setOnlinePlayerRef(event.target);
  ...
  const onPlayerStateChange = (event) => {
    if (event.data === 1) { // playing
      ...
      setIsOnlinePlaying(false);
      stopPolling();
      setOnlineCurrentTime(onlineDuration);
      playNextOnlineSong();
    }
  };
  ...
  const handleSeek = (e) => {
    if (!playerRef.current || !onlineDuration) return;
    const bar = e.currentTarget;
    const rect = bar.getBoundingClientRect();
    const ratio = Math.max(0, Math.min(1, (e.clientX - rect.left) / rect.width));
    const seekTime = ratio * onlineDuration;
    playerRef.current.seekTo(seekTime, true);
    setOnlineCurrentTime(seekTime);
  };
  ...
  const seekToLine = (index) => {
    ...
    setActiveLine(index);
    setOnlineCurrentTime(syncedLyrics[index].time);
  };
  ...
  const progress = onlineDuration > 0 ? (onlineCurrentTime / onlineDuration) * 100 : 0;

  return (
    <>
      {/* Hidden YouTube Player - stays mounted to keep audio playing in background */}
      {youtubeId && (
                ...
                <div className="flex-grow flex flex-col items-center justify-center w-full my-auto py-8">
                  ...
                  <div className="w-full max-w-[280px] mb-8">
                    ...
                    <div className="flex justify-between mt-2 text-xs font-bold text-slate-500 dark:text-zinc-500">
                      <span>{formatTime(onlineCurrentTime)}</span>
                      <span>{formatTime(onlineDuration)}</span>
                    </div>
                  </div>
